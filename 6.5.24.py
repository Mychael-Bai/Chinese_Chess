...
class ChineseChess:
    ...
    def __init__(self):
        ...
        self.window = tk.Tk()
        self.window.title("Chinese Chess 6.5.21(maybe strong but consumes too much time)")
    ...
    def evaluate_position_simple(self):
        """Simplified position evaluation for better performance"""
        piece_values = {
            '將': 6000, '帥': 6000,
            '車': 600,
            '馬': 270,
        ...
        for row in range(10):
            for col in range(9):
                ...
                if piece:
                    ...
                    # Simplified position bonuses
                    if piece[1] in ['卒', '兵']:  # Pawns
                        if piece[0] == 'B':
                            position_bonus = row * 10  # Bonus for advanced pawns
                        else:
                            position_bonus = (9 - row) * 10
    ...
    def minimax(self, depth, alpha, beta, maximizing_player, is_pv_node=True):
        """Enhanced minimax with Principal Variation search"""
        if depth == 0:
        ...
        if maximizing_player:
            ...
            for i, (from_pos, to_pos) in enumerate(moves):
                ...
                if not self.is_in_check('black'):
                    if i == 0 or not is_pv_node:
                        eval = self.minimax(depth - 1, alpha, beta, False, is_pv_node)
                    else:
                        # Scout search with null window
                        eval = self.minimax(depth - 1, alpha, alpha + 1, False, False)
                        if alpha < eval < beta:
                            eval = self.minimax(depth - 1, alpha, beta, False, True)
            ...
            min_eval = float('inf')
            moves = self._sort_moves(self.get_all_valid_moves('red'), 'red')
            ...
            for i, (from_pos, to_pos) in enumerate(moves):
                ...
                if not self.is_in_check('red'):
                    if i == 0 or not is_pv_node:
                        eval = self.minimax(depth - 1, alpha, beta, True, is_pv_node)
                    else:
                        # Scout search with null window
                        eval = self.minimax(depth - 1, beta - 1, beta, True, False)
                        if alpha < eval < beta:
                            eval = self.minimax(depth - 1, alpha, beta, True, True)
    ...
    def _sort_moves(self, moves, color):
        """Optimized move sorting for better pruning"""
        move_scores = []
        for from_pos, to_pos in moves:
            score = 0
            moving_piece = self.board[from_pos[0]][from_pos[1]]
            captured_piece = self.board[to_pos[0]][to_pos[1]]  # Fixed line
            ...
            if 3 <= to_pos[1] <= 5 and 3 <= to_pos[0] <= 6:
                score += 20
            
            # Prioritize developing moves
            if moving_piece[1] in ['馬', '炮', '車']:
                score += 15
    ...
    def make_ai_move(self):
        ...
        max_time = 50.0  # Reduced from 180.0 to 5 seconds per move
        ...
        # Start with depth 1 and increase progressively
        for current_depth in range(1, 6):  # Reduced max depth from 6 to 4
            ...
            for from_pos, to_pos in moves:
                ...
                # Only evaluate if the move doesn't put own king in check
                if not self.is_in_check('black'):
                    # Use minimax to evaluate the move
                    eval = self.minimax(current_depth - 1, float('-inf'), float('inf'), False)
                    
                    if eval > temp_best_score:
                        temp_best_score = eval
                        temp_best_move = (from_pos, to_pos)
