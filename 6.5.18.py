...
class OpeningBook:
    def __init__(self):
        # Common Chinese Chess openings
        # Format: list of moves [(from_pos, to_pos), ...]
        self.openings = {
            "中炮对直车": [
                # Black's responses to Red's central cannon
                ((2, 1), (4, 1)),  # 炮二平五
                ((0, 1), (2, 0)),  # 马二进三
                ((3, 0), (4, 0)),  # 卒三进一
            ],
            "中炮对横车": [
                # Another common defense against central cannon
                ((0, 8), (0, 6)),  # 车九平七
                ((0, 1), (2, 2)),  # 马二进三
                ((3, 2), (4, 2)),  # 卒三进一
            ],
            "仙人指路": [
                # Immortal points the way opening
                ((0, 1), (2, 2)),  # 马二进三
                ((2, 1), (4, 1)),  # 炮二平五
                ((3, 2), (4, 2)),  # 卒三进一
            ],
            "顺手炮": [
                # Screen horse cannon opening
                ((0, 1), (2, 2)),  # 马二进三
                ((2, 7), (2, 4)),  # 炮八平五
                ((3, 4), (4, 4)),  # 卒五进一
            ],
        ...
        # Store symmetrical variations
        self.symmetrical_openings = self._create_symmetrical_variations()
        
    def _create_symmetrical_variations(self):
        """Create symmetrical variations of openings (for both sides of the board)"""
        symmetrical = {}
        for name, moves in self.openings.items():
            symmetrical[name + "_对称"] = [
                ((row, 8-col), (new_row, 8-new_col))
                for (row, col), (new_row, new_col) in moves
            ]
        return symmetrical
        
    def get_opening_move(self, board, move_count):
        """Get the best opening move based on the current board position"""
        if move_count >= 6:  # Exit opening book after 6 moves
            return None
            
        current_position = self._get_board_state(board)
        
        # Check all openings and their variations
        for opening in {**self.openings, **self.symmetrical_openings}.items():
            if self._matches_opening(current_position, opening[1], move_count):
                return opening[1][move_count]
        
        return None
        
    def _get_board_state(self, board):
        """Convert current board state to a comparable format"""
        state = []
        for row in range(10):
            for col in range(9):
                if board[row][col]:
                    state.append((row, col, board[row][col]))
        return state
        
    def _matches_opening(self, current_position, opening_moves, move_count):
        """Check if current position matches the opening pattern up to move_count"""
        if move_count >= len(opening_moves):
            return False
            
        # Verify previous moves match the opening pattern
...
class ChineseChessOpenings:
    """Detailed Chinese Chess opening patterns"""
    
    def __init__(self):
        self.opening_patterns = {
            "飞相局": [
                # Famous Flying Elephant opening
                [(0, 2), (2, 4)],  # 象三进五
                [(0, 1), (2, 2)],  # 马二进三
                [(3, 0), (4, 0)],  # 卒三进一
            ],
            "系列炮": [
                # Series Cannon opening
                [(2, 1), (4, 1)],  # 炮二平五
                [(2, 7), (4, 7)],  # 炮八平五
                [(3, 4), (4, 4)],  # 卒五进一
            ],
        ...
        # Add common responses to Red's first moves
        self.responses = {
            "对中炮": {
                # Responses to Central Cannon
                "互进炮": [(2, 1), (4, 1)],  # Counter with own cannon
                "进马": [(0, 1), (2, 2)],    # Develop knight
                "进象": [(0, 2), (2, 4)]     # Develop elephant
            },
            "对起马": {
                # Responses to Knight's Opening
                "进马": [(0, 1), (2, 2)],
                "出车": [(0, 0), (0, 1)],
                "进炮": [(2, 1), (4, 1)]
            }
...
class ChineseChess:
    ...
    def identify_opening_position(self):
        """Identify current opening stage and position"""
        red_moves = self.get_red_moves_history()
        
        # Check if still in opening stage (usually first 10 moves)
        if len(red_moves) > 10:
            return None, None
            
        # Identify Red's opening strategy
        opening_type = self._identify_red_strategy(red_moves)
        move_number = len(red_moves)
        
        return opening_type, move_number

    def get_best_opening_response(self, opening_type, move_number):
        """Get best response based on opening type and move number"""
        if opening_type in self.opening_book.responses:
            responses = self.opening_book.responses[opening_type]
            # Choose response based on position evaluation
            best_response = None
            best_score = float('-inf')
            
            for response_name, moves in responses.items():
                if move_number < len(moves):
                    move = moves[move_number]
                    # Evaluate position after response
                    score = self.evaluate_opening_response(move)
                    if score > best_score:
                        best_score = score
                        best_response = move
    ...
    def make_ai_move(self):
        ...
        """Enhanced AI move selection with opening book"""
        # Initialize opening book if not exists
        if not hasattr(self, 'opening_book'):
        ...
        if opening_move:
            # Make the opening book move
            from_pos, to_pos = opening_move
            moving_piece = self.board[from_pos[0]][from_pos[1]]
            
            # Verify move is valid
            if moving_piece and moving_piece[0] == 'B' and self.is_valid_move(from_pos, to_pos):
                self.board[to_pos[0]][to_pos[1]] = moving_piece
                self.board[from_pos[0]][from_pos[1]] = None
        ...
        # If no opening move available or invalid, use regular AI logic
