...
class ChineseChess:
    ...
    def draw_board(self):
        ...
        for col, num in enumerate(black_numbers):
            x = self.board_margin + col * self.cell_size
            y = self.board_margin - 15  # Position above the board
        ...
        for col, num in enumerate(red_numbers):
            x = self.board_margin + col * self.cell_size
            y = self.canvas_height - self.board_margin + 15  # Position below the board
            self.canvas.create_text(
                x, y,
                text=num,
                fill='red',
