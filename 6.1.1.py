...
class ChineseChess:
    ...
    def get_piece_position_descriptor(self, from_pos, piece):
        """
        Determine if a piece needs 前/后 descriptor based on other same pieces in the same column.
        For pieces like 炮 and 車, when there are two in the same column:
        - For red pieces (bottom side):
          - 前 = the one farther from the bottom (smaller row number)
          - 后 = the one closer to the bottom (larger row number)
        - For black pieces (top side):
          - 前 = the one farther from the top (larger row number)
          - 后 = the one closer to the top (smaller row number)
        """
        from_row, from_col = from_pos
        same_pieces = []
        piece_type = piece[1]
        
        # Find all pieces of the same type and color in the same column
        for row in range(10):
            current_piece = self.board[row][from_col]
            if current_piece and current_piece[1] == piece_type and current_piece[0] == piece[0]:
                same_pieces.append(row)
        
        # If only one piece, no descriptor needed
        if len(same_pieces) <= 1:
            return ""
        
        # Sort the rows to find the relative positions
        same_pieces.sort()
        
        if piece[0] == 'R':  # Red pieces
            if piece_type in ['炮', '車']:  # For cannons and chariots
                # If it's the piece with smaller row number (farther from bottom), it's "前"
                if from_row == same_pieces[0]:
                    return "前"
                else:
                    return "后"
            else:  # For other pieces, keep the original logic
                return "前" if from_row < max(same_pieces) else "后"
        else:  # Black pieces
            if piece_type in ['炮', '車']:  # For cannons and chariots
                # If it's the piece with larger row number (farther from top), it's "前"
                if from_row == same_pieces[1]:
                    return "前"
                else:
                    return "后"
            else:  # For other pieces, keep the original logic
                return "前" if from_row > min(same_pieces) else "后"
