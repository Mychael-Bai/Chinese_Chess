...
class ChineseChess:
    ...
    def get_piece_position_descriptor(self, from_pos, piece):
        ...
        for row in range(10):
            ...
            if current_piece and current_piece == piece:
                same_pieces_in_col.append(row)
        
        # If only one piece, no descriptor needed
        if len(same_pieces_in_col) <= 1:
            return ""
            
        # For red pieces
        if piece[0] == 'R':
            # If the piece is farther from bottom (smaller row number), it's "前"
            return "前" if from_row < max(same_pieces_in_col) else "后"
        # For black pieces
        else:
            # If the piece is farther from top (larger row number), it's "前"
            return "前" if from_row > min(same_pieces_in_col) else "后"        
    ...
    def get_move_text(self, from_pos, to_pos, piece):
        ...
        if piece[0] == 'R':  # Red player
            ...
            from_col_text = columns[8 - from_col]  # Reverse for red side
        ...
        # Create move text starting with piece name and position descriptor
        move_text = f"{position_descriptor}{piece_name}{from_col_text}"
        
        # handling for 兵, 卒, 帥, 將, 車, 炮 (using steps)
        if piece_name in ['兵', '卒', '帥', '將', '車', '炮']:
            if piece[0] == 'R':  # Red player
                if to_row < from_row:  # Moving forward (up)
                    direction = '进'
                    steps = abs(to_row - from_row)
                    move_text += f"{direction}{columns[steps-1]}"
                elif to_row > from_row:  # Moving backward (down)
                    direction = '退'
                    steps = abs(to_row - from_row)
                    move_text += f"{direction}{columns[steps-1]}"
                else:  # Moving horizontally
                    direction = '平'
                    move_text += f"{direction}{to_col_text}"
            else:  # Black player
                if to_row > from_row:  # Moving forward (down)
                    direction = '进'
                    steps = abs(to_row - from_row)
                    move_text += f"{direction}{steps}"  # Use Arabic numbers for black
                elif to_row < from_row:  # Moving backward (up)
                    direction = '退'
                    steps = abs(to_row - from_row)
                    move_text += f"{direction}{steps}"  # Use Arabic numbers for black
                else:  # Moving horizontally
