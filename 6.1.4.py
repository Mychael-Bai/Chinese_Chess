...
class ChineseChess:
    ...
    def get_piece_position_descriptor(self, from_pos, piece):
        ...
        For cannons in the same column:
        - Red player (sitting at bottom): the one closer to red player (larger row number) is 后, farther one is 前
        - Black player (sitting at top): the one closer to black player (smaller row number) is 后, farther one is 前
        """
        from_row, from_col = from_pos
        piece_color = piece[0]  # 'R' for red or 'B' for black
        piece_type = piece[1]   # '炮' for cannon
        
        # Special handling for cannons
        if piece_type == '炮':
            other_cannon_row = None
            # Search for another cannon of the same color in the same column
            for row in range(10):
                if row != from_row:  # Don't check the current cannon's position
                    current_piece = self.board[row][from_col]
                    if current_piece and current_piece[0] == piece_color and current_piece[1] == '炮':
                        other_cannon_row = row
                        break
            
            # If found another cannon in the same column
            if other_cannon_row is not None:
                if piece_color == 'R':  # Red player's perspective (sitting at bottom)
                    # If current cannon is closer to red player (larger row number)
                    return "后" if from_row > other_cannon_row else "前"
                else:  # Black player's perspective (sitting at top)
                    # If current cannon is closer to black player (smaller row number)
                    return "后" if from_row < other_cannon_row else "前"
            return ""  # No other cannon in the same column
        
        # For other pieces, keep the original logic
        piece_rows = []
        for row in range(10):
            current_piece = self.board[row][from_col]
