...
class ChineseChess:
    ...
    def get_move_text(self, from_pos, to_pos, piece):
        ...
        # Create move text starting with piece name and column
        move_text = f"{piece_name}{from_col_text}"
        
        # Special handling for 兵, 卒, 帥, 將
        if piece_name in ['兵', '卒', '帥', '將']:
            # Determine movement direction and steps
            if piece[0] == 'R':  # Red player
                if to_row < from_row:  # Moving forward (up)
                    direction = '进'
                    steps = abs(to_row - from_row)
                    move_text += f"{direction}{columns[steps-1]}"  # Convert steps to Chinese numbers
                ...
                if to_row > from_row:  # Moving forward (down)
                    ...
                    direction = '退'
                    steps = abs(to_row - from_row)
                    move_text += f"{direction}{steps}"
                else:  # Moving horizontally
                    direction = '平'
                    move_text += f"{direction}{to_col_text}"
        elif piece_name in ['相', '象', '馬']:  # Elephants and Horses use destination columns
            if piece[0] == 'R':  # Red player
                if to_row < from_row:
                    direction = '进'
                    move_text += f"{direction}{to_col_text}"
            ...
            # Original handling for other pieces
            if piece[0] == 'R':  # Red player
                if to_row < from_row:
                    direction = '进'
                    steps = abs(to_row - from_row)
    ...
    def add_move_to_history(self, from_pos, to_pos, piece):
        ...
        player_text = "红方" if piece[0] == 'R' else "黑方"
        self.move_history.append(f"{player_text}: {move_text}")
