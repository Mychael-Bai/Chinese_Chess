...
class ChineseChess:
    ...
    def __init__(self):
        ...
        self.window = tk.Tk()
        self.window.title("Chinese Chess 6.5.20(maybe strong but consumes too much time)")
    ...
    def make_ai_move(self):
        ...
        max_time = 180.0  # Maximum time allowed for the move
        ...
        # Iterative deepening from depth 1 to 6
        for current_depth in range(1, 7):
            temp_best_score = float('-inf')
            temp_best_move = None
            
            # Check remaining time
            if time.time() - start_time > max_time * 0.8:  # Use 80% of max time as cutoff
                break
            ...
            for from_pos, to_pos in moves:
                ...
                if not self.is_in_check('black'):
                    score = self.minimax(current_depth, alpha, beta, False)
            ...
            if temp_best_move and time.time() - start_time <= max_time:
                best_score = temp_best_score
                best_move = temp_best_move
                print(f"Completed depth {current_depth} search")  # Optional: for debugging
