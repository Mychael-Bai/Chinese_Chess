...
class ChineseChess:
    ...
    def __init__(self):
        ...
        self.window = tk.Tk()
        self.window.title("Chinese Chess 6.5.05(with sound)")
    ...
    def make_ai_move(self):
        """Make the best move for black pieces"""
        ...
        for from_pos, to_pos in moves:
            ...
            if not self.is_in_check('black'):
                ...
                if captured_piece and captured_piece[0] == 'R':
                    score += 50  # Bonus for capturing opponent's pieces
                
                # Add bonus for checking opponent
        ...
        if best_move:
            ...
            # Record the move
            moved_piece = self.board[from_pos[0]][from_pos[1]]
            
            # Make the move
            self.board[to_pos[0]][to_pos[1]] = moved_piece
            self.board[from_pos[0]][from_pos[1]] = None
            
            # Record the move in history
            self.add_move_to_history(from_pos, to_pos, moved_piece)
    ...
    def get_piece_position_descriptor(self, from_pos, to_pos, piece):
        ...
        if len(identical_positions) == 2:
            ...
            opponent_king_row = 0 if piece_color == 'R' else 9
        ...
        if self.move_text and self.move_history:
            ...
            for i, move in enumerate(self.move_history, 1):
                ...
                if i % 2 == 0:
                    num_space = int(i / 2)
                    space = ""
                    for j in range(len(str(num_space))):
                        space += " "
                    self.move_text.insert(tk.END, f"{space}  {move}\n")
                else:
                    number_list = int((i + 1) / 2)
                    self.move_text.insert(tk.END, f"{number_list}. {move}\n")
