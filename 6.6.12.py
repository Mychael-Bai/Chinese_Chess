...
class ChineseChess:
    ...
    def _move_sorting_score(self, move):
        ...
        if self.is_in_check('red'):
            score += 1000  # Much higher bonus for checks
            
        # Bonus for advanced position
        if from_piece[1] in ['車', '馬', '炮']:
            if from_piece[0] == 'B' and to_pos[0] > 4:  # Black pieces crossing river
                score += 50
            elif from_piece[0] == 'R' and to_pos[0] < 5:  # Red pieces crossing river
                score += 50
