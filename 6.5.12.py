...
class ChineseChess:
    ...
    def evaluate_position_strategic(self):
        """Advanced strategic position evaluation"""
        ...
        def evaluate_attack_potential(self, color):
            """Evaluate attacking strength and control of key positions"""
            attack_score = 0
            opponent = 'red' if color == 'black' else 'black'
            opp_king_pos = self.find_kings()[0 if color == 'black' else 1]
            
            if not opp_king_pos:
                return 1000
            
            opp_king_row, opp_king_col = opp_king_pos
            
            # Evaluate piece proximity to opponent's king
            for row in range(10):
                for col in range(9):
                    piece = self.board[row][col]
                    if piece and piece[0] == color[0].upper():
                        distance = abs(row - opp_king_row) + abs(col - opp_king_col)
                        attack_score += (10 - distance) * 5
            ...
            for col in range(9):
                ...
                if piece:
                    ...
                    if piece[0] == 'B':  # Black pieces (AI)
                        ...
                        # Positional bonuses
                        if piece[1] == '卒':  # Pawn
                            position_score += row * 15  # Advance pawns
                        elif piece[1] in ['馬', '炮']:  # Knights and Cannons
                            if 2 <= row <= 7 and 2 <= col <= 6:
                                position_score += 20  # Control center
                    else:  # Red pieces
                        material_score -= value
                        if piece[1] == '兵':
                            position_score -= (9 - row) * 15
        
        # Strategic evaluation
        king_safety = evaluate_king_safety(self, 'black') - evaluate_king_safety(self, 'red')
        attack_score = evaluate_attack_potential(self, 'black') - evaluate_attack_potential(self, 'red')
        ...
        final_score = (material_score * 1.0 +
                      position_score * 0.5 +
                      king_safety * 0.8 +
                      attack_score * 0.6)
    ...
    def minimax(self, depth, alpha, beta, maximizing_player):
        ...
        if depth == 0:
            return self.evaluate_position_strategic()
        
        if maximizing_player:
            max_eval = float('-inf')
            moves = self.get_all_valid_moves('black')
            moves.sort(key=lambda move: self._move_sorting_score(move), reverse=True)
            
            for from_pos, to_pos in moves:
                # Store pieces before move
                ...
                # Restore position exactly as it was
            ...
            moves.sort(key=lambda move: self._move_sorting_score(move))
    ...
    def make_ai_move(self):
        """Make the best move for black pieces using minimax"""
        ...
        for from_pos, to_pos in moves:
            # Store the pieces before making the move
            moving_piece = self.board[from_pos[0]][from_pos[1]]
            captured_piece = self.board[to_pos[0]][to_pos[1]]
            ...
            if not self.is_in_check('black'):
                score = self.minimax(3, alpha, beta, False)  # Reduced depth for faster response
                
                if score > best_score:
                    best_score = score
                    best_move = (from_pos, to_pos)
                    best_moving_piece = moving_piece  # Store the piece that's moving
        ...
        if best_move:
            ...
            # Verify the piece is still there
            if self.board[from_pos[0]][from_pos[1]] == best_moving_piece:
                ...
                # If clicking on a valid move position
                    ...
                    if self.is_in_check(self.current_player):
                        ...
                        if self.current_player == 'black':
                            ...
                            self.window.after(1200, self.make_ai_move)
