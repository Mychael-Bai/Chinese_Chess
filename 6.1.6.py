...
class ChineseChess:
    ...
    def get_piece_position_descriptor(self, from_pos, piece):
        ...
        if piece_type == '炮':
            # Find all cannons of the same color in the same column
            cannon_positions = []
            for row in range(10):
                current_piece = self.board[row][from_col]
                if current_piece and current_piece[0] == piece_color and current_piece[1] == '炮':
                    cannon_positions.append(row)
            
            # If there are two cannons in the same column
            if len(cannon_positions) == 2:
                if piece_color == 'R':  # Red player's perspective (sitting at bottom)
                    return "后" if from_row == max(cannon_positions) else "前"
                else:  # Black player's perspective (sitting at top)
                    return "后" if from_row == min(cannon_positions) else "前"
            
            # Important: Return an empty string if there's only one cannon
            # This will force the notation to use the column number instead
            return ""
        
        # For other pieces, return empty string to use default notation
        return ""
    ...
    def get_move_text(self, from_pos, to_pos, piece):
        ...
        if position_descriptor and piece_name == '炮':
