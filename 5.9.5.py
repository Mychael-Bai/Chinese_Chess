...
class ChineseChess:
    ...
    def get_move_text(self, from_pos, to_pos, piece):
        ...
        # Special handling for 相, 象, 馬 (using destination columns)
        elif piece_name in ['相', '象', '馬']:
            if piece[0] == 'R':  # Red player
                if to_row < from_row:
                    direction = '进'
                    move_text += f"{direction}{to_col_text}"
        ...
        # Original handling for other pieces (炮, 車)
        else:
            if piece[0] == 'R':  # Red player
                if to_row < from_row:
                    direction = '进'
