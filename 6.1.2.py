...
class ChineseChess:
    ...
    def get_piece_position_descriptor(self, from_pos, piece):
        ...
        For cannons and chariots:
        - For red player (bottom side):
          - 后 = the one closer to bottom (larger row number)
          - 前 = the one farther from bottom (smaller row number)
        - For black player (top side):
          - 后 = the one closer to top (smaller row number)
          - 前 = the one farther from top (larger row number)
        """
        ...
        # For cannons (炮) and chariots (車)
        if piece_type in ['炮', '車']:
            if piece[0] == 'R':  # Red pieces
                # The one with larger row number (closer to bottom) is "后"
                return "前" if from_row == max(same_pieces) else "后"
            else:  # Black pieces
                # The one with smaller row number (closer to top) is "后"
